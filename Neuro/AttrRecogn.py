...
class MammalNeuron(Neuron):
    def __init__(self):
        super().__init__(weights=[4.0, 0.01, 0.01, -1.0, -1.5], activator=lambda x: x)


class BirdNeuron(Neuron):
    def __init__(self):
        super().__init__(weights=[2.0, -1.0, 2.0, 2.5, 2.0], activator=lambda x: x)


class FishNeuron(Neuron):
    def __init__(self):
        super().__init__(weights=[-1.0, 3.5, 0.01, -2.0, 1.5], activator=lambda x: x)
...
winner_chain, result = selector.run([[4.0, -1.0, -1.0, -1.0, -1.0]] * 3)
assert (winner_chain[-1].neuron.__class__.__name__, result) == ("MammalNeuron", 18.48)

winner_chain, result = selector.run([[2.0, -1.0, 2.0, 2.0, 1.0]] * 3)
assert (winner_chain[-1].neuron.__class__.__name__, result) == ("BirdNeuron", 16.0)

winner_chain, result = selector.run([[0.0, 2.0, -1.0, -1.0, 2.0]] * 3)
